...
class Rediser():
    ...
    def get_embeds(self):
        embeds = self.db.lrange(self.settings.EMBED_QUEUE,0,-1)
    ...
    def get_labels(self):
        return np.array(self.db.lrange(self.settings.LABEL_QUEUE,0,-1)).astype(str)

    def add_embeds(self, embeds, labels):
        base64_embeds = [base64_encode_image(e) for e in embeds]
        for i in range(len(base64_embeds)):
            self.db.rpush(self.settings.EMBED_QUEUE, base64_embeds[i])
            self.db.rpush(self.settings.LABEL_QUEUE, labels[i])
    ...
    def remove_student(self, student_id):
        ...
        #delete student
        [self.db.lset(self.settings.EMBED_QUEUE, int(idx), "REMOVE") for idx in student_indexes]
        embed_remove = self.db.lrem(self.settings.EMBED_QUEUE, 0, "REMOVE")
        label_remove = self.db.lrem(self.settings.LABEL_QUEUE, 0, student_id)
